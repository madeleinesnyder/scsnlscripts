...
def submit_AROMA_job(subject_path,slurm_args,brainmask,pipeline,aggr_sign):
        '''
            Added by JN to run on sherlock 7/17/17
        '''
        ...
        callstr = ('sbatch'
                   ...
                   ' --mem-per-cpu=%s'
                   ' -p %s'
                   ' /oak/stanford/groups/menon/scsnlscripts/ica-aroma/submit_job.sh %s %s %s %s')%(slurm_args['job_name'],
	...
	print(callstr)
        #callstr = ("python /oak/stanford/groups/menon/scsnlscripts/ica-aroma/run_AROMA.py %s %s %s %s"%(subject_path,brainmask,pipeline,aggr_sign))
