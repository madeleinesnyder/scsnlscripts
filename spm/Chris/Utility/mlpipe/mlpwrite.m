function count=mlpwrite(pid,data,precision,machineformat);
% count=mlpwrite(pid,data [,precision,machineformat]);
...
switch (machineformat)
  ...
  otherwise
    error('(popen) Unsupported machine format!')
end

% call mlpmex
count=mlpmex(pid,'write',data,nbytes,datatype,byteswap);
