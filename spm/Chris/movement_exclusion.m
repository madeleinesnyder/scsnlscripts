function movement_exclusion(ConfigFile)
...
SubjList                 = strtrim(config.subject);
PrepFolder           = strtrim(config.preprocessedfolder);
CondList              = strtrim(config.sessionlist);
ScanToScanCrit = config.scantoscancrit;
...
for iSubj = 1:NumSubjs
  for iCond = 1:NumConds
    ...
    MvmntDir{RunCnt} = fullfile(project_dir,'/data/imaging/participants',YearId, Subjects{iSubj}, 'fmri', ...
    ...
    if exist(ZippedMvmntFile, 'file') || exist(ZippedGSFile, 'file')
      ...
      if ~exist(MvmntFile, 'file') || ~exist(GSFile, 'file')
        ...
        CondStatsFile = fullfile(project_dir,'/data/imaging/participants',YearId, Subjects{iSubj}, 'fmri', ...
