function movement_exclusion_sumabs(Config_File)
  ...
  for iCond = 1:NumConds
    ...
    if exist(ZippedMvmntFile, 'file') || exist(ZippedGSFile, 'file')
      ...
      if ~exist(MvmntFile, 'file') || ~exist(GSFile, 'file')
        ...
        MvmntStats(RunCnt, 9) = sum(ScanToScanTotalDisp > (ScanToScanCrit*VoxSize));
        ...
        MvmntStats(RunCnt, 10) = length(glout_idx);
        
        union_idx = unique([1; mvnout_idx(:); glout_idx(:)]);
        MvmntStats(RunCnt, 11) = length(union_idx)/length(g)*100;
...
if ~isempty(FullRunIndex)
  fid = fopen('MovementSummaryStats_Sum_Abs.txt', 'w+');
  fprintf(fid, '%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n', 'TASK', 'Scan_ID', ...
    'Range x', 'Range y', 'Range z', 'Range pitch', 'Range roll', 'Range yaw', 'Max Displacement', ...
    'Max Scan-to-Scan Displacement', ['Num Scans > ', num2str(ScanToScanCrit*VoxSize), ' mm'], ...
