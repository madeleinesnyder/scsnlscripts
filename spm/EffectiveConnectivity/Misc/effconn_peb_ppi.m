function PPI = effconn_peb_ppi(varargin)

% Modified from spm_ped_ppi.m for effconn_ppi.m batch script
...
%--------------------------------------------------------------------------
Finter = spm_figure('CreateWin','PPI','PPI Setup', 'off');
header = get(Finter,'Name'); set(Finter,'name','PPI Setup');
...
if spm_matlab_version_chk('7') >= 0,
    save(fullfile(ppi_dir, str),'-V6','PPI')
else
    save(fullfile(ppi_dir, str),'PPI')
end

cd(ppi_dir);
saveas(gcf, 'PPI_Setup.jpg');
% Clean up
%--------------------------------------------------------------------------
%spm('Pointer','arrow'); 
