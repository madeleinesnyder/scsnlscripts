function SPM=PPPI_mask(parameterstructure,structfile, mask_file)
      ...
      if strcmp(P.method,'trad')
        ...
        for i=1:numel(T)
            for j=1:length(Sess.U(T(i)).name)
                %if any(Sess.U(T(i)).u(33:end,j)):-comment out checking
                %empty condition -TC
        ...
        for i=1:numel(T)
            for j=1:length(Sess.U(T(i)).name)
                %if any(Sess.U(T(i)).u(33:end,j))- commenting out checking
                %for empty condition -TC
                    U.u             = [U.u Sess.U(T(i)).u(33:end,j)];
                    U.name{end + 1} = Sess.U(T(i)).name{j};
                    U.w             = 1;
                %end
...
if P.Estimate==1
   try
        SPM=spm_estimate_PPI_mask(P.subject,'SPM.mat',P.Region,P.method,P.analysis,P.CompContrasts, mask_file);
   catch
