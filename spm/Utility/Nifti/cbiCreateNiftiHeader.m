...
%       d) if qform44 is empty or identity, qform_code=0, else qform_code=1
...
%       d) if sform44 is empty or identity, sform_code=0, else sform_code=1
...
% Options 2-4 can be combined. The data and header input arguments must come before the parameter-value pairs.
  ...
  if (use_qform)
    % Make sure qform44 is valid. Identity is allowed!
    if (~isempty(hdr.qform44) & ~isequal(hdr.qform44,zeros(4,4)) & size(hdr.qform44)==[4,4])
  ...
  % Set qform_code, sform_code according to header if not already set
  NIFTI_XFORM_CODES=1:10; % Currently defined coordinate systems only 1-4, but we allow for extensions
  % qform44:   
  if (~isempty(hdr.qform44))
      if (isempty(hdr.qform_code) | ~any(hdr.qform_code==NIFTI_XFORM_CODES))
          hdr.qform_code=1;
      end
  else
    hdr.qform_code=0;
  ...
  if (isfield(hdr,'hdr_name'))
      ...
      [pathstr,imgname,hdrname,ext,ver,compext,iscomp,ishdrcomp]=cbiParseNiftiFilename(hdr.hdr_name);
    switch (ext)
     case '.nii'  
       hdr.single_file=1;
       hdr.magic='n+1';
