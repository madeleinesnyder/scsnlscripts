...
function discard_vols_CNI (Config_File)
...
for iSubj = 1:NumSubj
  ...
  for iSess = 1:NumSess
    ...
    if exist(DataDir, 'dir')
        unix(sprintf('/bin/rm -rf %s', DataDir));
    end
    mkdir(DataDir);
    cd(DataDir);
    cd ..;
    ListFile = dir(fullfile(pwd, '*.nii.gz'));
    if isempty(ListFile)
      ...
      if length(ListFile) > 1
        ...
        unix(sprintf('cp -a %s %s', ListFile(1).name, fullfile(DataDir, 'I.nii.gz')));
        cd(DataDir);
        nifti4Dto3D(DataDir, []);
        ListFile = dir('*.nii');
