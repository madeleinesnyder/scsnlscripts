...
ServerPath         = strtrim(config.projectdir);
...
if ~isempty(MatchTaskName)
    msize = size(MatchTaskName);
    ncols = msize(1)/2;
    temp_matchtaskname = {};
    for ii = 1:ncols
      n = MatchTaskName(ii+(ii-1),:);
     val = str2num(MatchTaskName(ii+ii,:));
     temp_matchtaskname{ii} = {n,val};
    end
    MatchTaskName = temp_matchtaskname;
...
for iSubj = 1:NumSubj
  ...
  for iSess = 1:NumSess
    fprintf('---> Session: %s\n', Sessions{iSess});
    DataDir{SessCnt} = fullfile(ServerPath, 'data/imaging/participants',YearId, Subjects{iSubj}, 'fmri', ...
