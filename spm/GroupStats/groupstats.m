...
output_folder    = strtrim(config.output_dir);
...
output_folder = fullfile(stats_path,'/results/',fmri_type,output_folder);
...
% Get number of groups (how many subject.csv files there are at once)

subjectlist_size = size(subjectlist);
numgroup = subjectlist_size(1);
input_subjectlist = subjectlist;
subjectlist = {};
for list_i = 1:numgroup
    subjectlist{list_i} = strtrim(input_subjectlist(list_i,:));
end
...
if(exist(subjectlist{1}, 'file') == 0)
  fprintf('File does not exist: %s\n', subjectlist{1});
  diary off; return;
end

if(numgroup == 2)
  if(exist(subjectlist{2}, 'file') == 0)
    fprintf('File does not exist: %s\n', subjectlist{2});
    diary off; return;
  end
end
...
subjects1   = csvread(input_subjectlist,1);
batchfile   = 'batch_1group.mat';
numsubg1    = length(subjects1)-1;

if(numgroup == 2)
  subjects2 = ReadList(subjectlist{2});
...
if isempty(parent_dir{1})
  for subcnt1 = 1:numsubg1
    subject1 = subjects1(subcnt1,1);
    subject1 = char(pad(string(subject1),4,'left','0'));
    visit = string(subjects1(subcnt1,2));
    visit = char(strcat('visit',visit));
    session = string(subjects1(subcnt1,3));
    session = char(strcat('session',session));
    sub_stats1{subcnt1} = fullfile(stats_path, subject1, visit, session, ...
                                   'fmri', 'stats_spm8', stats_dir);
  end
  if numgroup == 2
    for subcnt2 = 1:numsubg2
       subject2 = subjects2(subcnt2,1);
       subject2 = char(pad(string(subject2),4,'left','0'));
       visit = string(subjects2(subcnt2,2));
       visit = char(strcat('visit',visit));
       session = string(subjects2(subcnt2,3));
       session = char(strcat('session',session));
       sub_stats2{subcnt2} = fullfile(stats_path, subject2, visit, session, ...
                                   'fmri', 'stats_spm8', stats_dir);
    end
  end
  else
