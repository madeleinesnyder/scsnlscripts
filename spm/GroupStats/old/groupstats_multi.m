...
function groupstats_multi(Config_File)
...
stats_parent     = strtrim(paralist.stats_parent);
stats_children   = strtrim(paralist.stats_children);
output_parent    = strtrim(paralist.output_parent);
...
for stats_folder = stats_children
...
if isempty(parent_folder{1})
  for subcnt1 = 1:numsubg1
    ...
    sub_stats1{subcnt1} = fullfile(stats_path, pfolder, subjects1{subcnt1}, ...
                                   'fmri', 'stats_spm8', stats_parent, stats_folder{1});
  end
  if numgroup == 2
...
output_folder = fullfile(output_parent,stats_folder{1});
mkdir(output_folder);
cd(output_folder);
