...
% /home/fmri/fmrihome/SPM/spm8_scripts/GroupStats/old_groupstats_config.m.template
% 
% This group-stats scripts are compatible with both Analyze and NIFTI formats
% To use either format, change the data type in old_groupStats_config.m.template
%
...
% >> old_groupstats('groupstats_config.m')
%
% _________________________________________________________________________
% 2009 Stanford Cognitive and Systems Neuroscience Laboratory
%
% $Id: old_groupstats.m 2009-07-20 $
% -------------------------------------------------------------------------

function old_groupstats(Config_File)
...
contrastfile = fullfile(participant_path,subjects1{1},stats_folder,'contrasts.mat');
load(contrastfile);
  ...
  % -----------------------------------------------------------------------
   if(numgroup == 1)               
    if ~isempty(reg_vec)
      ...
      for j=1:numsubg1
        file = fullfile(participant_path, subjects1{j},stats_folder, ...
               ['/con_0' conNum '.img,1']);
     ...
     for j=1:numsubg2
       file = fullfile(participant_path, subjects2{j}, stats_folder, ...
                      ['/con_0' conNum '.img,1']);
    ...
    matlabbatch{2}.spm.stats.fmri_est.spmmat{1}      = [pwd, '/SPM.mat'];
    matlabbatch{3}.spm.stats.con.spmmat{1}           = [pwd, '/SPM.mat'];
