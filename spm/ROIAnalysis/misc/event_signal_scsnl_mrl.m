function [s, varargout] = event_signal_scsnl(D, e_spec, dur, diff_func, varargin)
...
%
% $Id: event_signal.m 542 2004-12-14 08:40:04Z matthewbrett $ 
...
for e_i = 1:1 %e_s_l
  es    = e_specs(:, e_i);
  ss    = es(1);
  Yh    = event_fitted_scsnl(D, es, dur);
  d     = pr_ev_diff_scsnl(Yh, diff_func, varargin{:});
  s     = s + d ./ s_mus(ss,:);
end
s = s / e_s_l * 100;
varargout{1} = Yh;