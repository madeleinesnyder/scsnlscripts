function d = pr_ev_diff_scsnl(ev_tc, diff_func, varargin)
...
%                              otherwise => (min - max) 
%  
% $Id: pr_ev_diff.m 490 2004-09-27 18:09:04Z matthewbrett $
...
switch lower(diff_func)
 case 'signed max-min'
   if(abs(max(ev_tc, [], 1)) > abs(min(ev_tc, [], 1)))
     sign = 1;
   else
     sign = -1;
   end
   d = (max(ev_tc, [], 1) - min(ev_tc, [], 1)) * sign;  
 case 'max'
  d = max(ev_tc, [], 1);
 case 'max-min'
