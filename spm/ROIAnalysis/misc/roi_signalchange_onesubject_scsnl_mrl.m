function [signalchange] = roi_signalchange_onesubject_scsnl(ROIs, subject_stats_dir, event_duration)
...
%
%
% author: Catie Chang 9/26/2006, modified VMenon 2006/09, 2006/10
%
% modified, Sridhar: 21 Nov to 04 Dec, 2007 to use probabilistic rois and 
% events of different durations, as well as some general cleanup
% 
% Last modified by Kumar: Aug 27, 2008.  The argument is_similar_multisession 
% is removed  from the parameter list.  The IF-ELSE condition based on 
% that variable is also modified.  After this change, all the sessions are 
% treated as NOT SIMILAR sessions.
...
%            end
            ...
            if DEFAULT_EVENT  % Use default SPM event duration or different durations
              for d=1:length(durs),	
                [pcs(d), ts{d}] = event_signal_scsnl(E, e_specs(:,e_s), durs(d), event_string);
                LEN = max(LEN, length(ts{d}));
              end % for d	
            else
                % Use specified event window
              for d=1:length(durs),	
                [pcs(d), ts] = event_signal_scsnl(E, e_specs(:,e_s), durs(d), 'window', event_duration, bf_dt(E));
                LEN = max(LEN, length(ts{d}));
              end % for d
            end
            signalchange.data_roi_sess_event{r}{s}(e) = mean(pcs);
            signalchange.voxel_data{r}{s}{e} = pcs
