function KendallMatrix2()
table1B();
%table2();
end
function table1B(), % DO J=1 here. 
s(1).dir = '../hcp/hcp_LDS_1';
s(2).dir = '../Yale/yale_LDS_1';
s(3).dir = '../StanfordStopGo/StopGo_LDS_1';
s(4).dir = '../oddball/oddball_LDS_1';
s(5).dir = '../Luna/luna_LDS_1';
...
for i=1:length(s),
    %%
    ij = [2,2];
    Kij = s(i).x.D{end,3}.K(ij(1), ij(2));    
    dM = {}; 
    %v = permute(cell2mat({(s(i).sim(1,:).(fld)) }) , [2, 3, 1]);
    v = cellfun(@(c)max(c.K(:)), s(i).x.D(end,:)); 
    v = [v' [v(1:2)' ; Kij]];
...
%%
NAMES{end+1} = 'LAST';
save('kendall_tbl1', 'NAMES','SIMS','M','B','H','W');
matlab_jinjafy('kendall_table1_partial.tex');
%%
close all;
S = length(s);
for i=1:length(s),
    subplot(1,S,i);
    imagesc(s(i).x.D{end,end}.K);
    axis image;
    title(s(i).x.name);
end
figure(2);
%%
close all;
f = figure('Units','Centimeters','Position',[4, 4, 20, 30]);
cc = 1; 
for i=1:length(s),
    
    for m=1:3,
        subplot(S,3,cc);
        v = s(i).x.D{end,m}.C_best;
        v = reshape(v, [1, 1, numel(v)]);
        v = cell2mat(v);
        v = rd(mean(v,3));
        v = v + median(v(:)) * eye(size(v,1));
        imagesc(v)
        cc = cc+1;
        axis image;
        ss = sprintf('%s-%s: %2.3g',NAMES{i}, dSIMS{m}, 100*M(i,m));
        title(ss);
    end
end
addpath('../../matlabfrag/');
mlf2pdf(f, '../Latex/figures/allmatrices');
%%
end 
 
function table2() % do J=2 results here. 
disp('LOOK AT KENDELLMATRIX.M');
assert(false);
