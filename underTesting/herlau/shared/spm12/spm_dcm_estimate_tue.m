function [DCM] = spm_dcm_estimate_tue(P)
...
odef.random_init = false;
if ~isfield(DCM,'opts'),
    DCM.opts = struct();
end
DCM.opts = ssfr(odef,DCM.opts);
...
if ~DCM.options.stochastic
    ...
    %----------------------------------------------------------------------
%     M.pE.A(M.pE.A>0) = (rand(1,nnz(M.pE.A))-.5)/10;
%     M.pE.B(M.pE.B>0) = (rand(1,nnz(M.pE.B))-.5)/10;
%     M.pE.C(M.pE.C>0) = (rand(1,nnz(M.pE.C))-.5)/10;
    
    [Ep,Cp,Eh,F] = spm_nlsi_GN(M,U,Y);
    assert(false); % are we really sure about this?
    ...
    % ---------------------------------------------------------------------
    if DCM.options.random_init,
        M.pE.A(M.pE.A>0) = (-rand(1,nnz(M.pE.A)))/2;
        M.pE.B(M.pE.B>0) = (rand(1,nnz(M.pE.B))-.5)/2;
        M.pE.C(M.pE.C>0) = (rand(1,nnz(M.pE.C))-.5)/2;
    end
    ...
    subplot(4,1,3); plot(Y.y);
    subplot(4,1,4); plot(Y.y, '-'); hold on;
    plot(y, '.');    
    %%
end
